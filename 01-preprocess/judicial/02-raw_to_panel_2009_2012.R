...
# these are the municipalities of 2025
#munpan <- read_parquet('../../../data/00-map/municipalities.parquet')

# These are the municipalities of 2012
...
preprocess_judicial_2009 <- function(year, panel = T){
  ...
  pdel <- read.dbf(paste0(pf, 'Judiciales_BD_Catalogos_', year, 
                          '_dbf/TablasMicrodatos_', year, '/pdel', year, 
                          '.DBF'), as.is = T) # Processed crimes
  sreg <- read.dbf(paste0(pf, 'Judiciales_BD_Catalogos_', year, 
                          '_dbf/TablasMicrodatos_', year, '/sreg', year, 
                          '.DBF'), as.is = T) # Registry: Sentenced
  sdel <- read.dbf(paste0(pf, 'Judiciales_BD_Catalogos_', year, 
                          '_dbf/TablasMicrodatos_', year, '/sdel', year, 
  ...
  pdel.ind <- pdel.ind |> 
    mutate(date_ocu = dmy(B_FOCU), # Date of Ocurrence of crime
  ...
  sdel.w <- sdel.w |> mutate(snieg_code = paste0('crime_sent_', snieg_code)) |>
    select(ID_PS, snieg_code) |>
    mutate(dummy = 1)
  ...
  if (panel == F){
    ...
    preg.agg.p <- preg.final |> mutate(date_auto = date_auto - 3, #72 hours before the judge decides
                                       month_auto = month(date_auto)) |>
      filter(is.na(code_inegi_rh) == F) |> filter(federal == 1) |>
      filter(man == 1)
    ...
    final.panel <- final.panel |> left_join(preg.agg, 
                                            by = c('code_inegi' = 'code_inegi_rh', 
                                                   'month' = 'month_auto')) |>
      mutate(across(c(formal_prision_marg:petty), 
    ...
    sreg.agg <- sreg.agg |>
      mutate(sent_prison_marg = sent_prison*marg_condition, 
             ...
             sent_prison_drugs = sent_prison*crime_sent_8,
