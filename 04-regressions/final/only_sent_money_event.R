...
event_general <- function(df, treat_post_p, time_period ='bimonthly', 
  ...
  if(time_period == 'monthly'){
    ...
    df <- df |> group_by(code_inegi, year, bimonthly) |> 
      summarise(across(c(crime_5, only_sent_money), ~sum(.x))) |> 
  ...
  df <- df |> mutate(only_sent_money = log(only_sent_money + 1))
...
do_plot(size_point = 1, size_titles = 13, time_b = -50, 
        time_a = 60, ylimb = .3, ylima = 0.2, var = 'only_sent_money')
