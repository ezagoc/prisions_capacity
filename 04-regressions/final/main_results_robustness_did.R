...
pacman::p_load(tidyverse, arrow, purrr, sf, fixest, didimputation, did, 
               did2s, fect, patchwork, DIDmultiplegtDYN)
...
funct_atts <- function(variable){
  ...
  fmla <- as.formula(paste0(variable, ' ~ sunab(event_time,
                                                 m_time) ',
                            ' | code_inegi + m_time'))
  reg200k <- feols(fmla, cluster = "code_inegi", data = p)
  
  att_row <- summary(reg200k, agg = "ATT")$coeftable["ATT", ]
  
  sunab_tibble <- tibble(estimate = att_row["Estimate"],
                         se = att_row["Std. Error"], 
                         type = 'SunAb') |>
    mutate(conf.low = estimate - se*1.96,
  ...
  borus <- did_imputation(data = p, yname = variable, 
                                gname = "event_time",
                                tname = "m_time", idname = "code_inegi") |> 
    mutate(type = "didimputation") |> 
    select(type, estimate, se = std.error) |>
    mutate( conf.low = estimate - se*1.96, 
            conf.high = estimate + se*1.96, 
            conf.low1 = estimate - se*1.645, 
            conf.high1 = estimate + se*1.645, 
  ...
  # ch_dh <- did_multiplegt_dyn(df = p, outcome = variable, group = "code_inegi", 
  #                             time = "m_time", 
  #                             treatment = "treat_post",
  #                             cluster = 'code_inegi', graph_off = F, 
  #                             effects = 10,
  #                             placebo = 10)
  # 
  # df_ch_dh <- tibble(type = "DIDmultiplegt", variable = variable,
  #                    estimate = ch_dh[["coef"]]$b, 
  #                    se = sqrt(ch_dh[["coef"]]$vcov[1]), 
  #                    conf.low = estimate - se*1.96, 
  #                    conf.high = estimate + se*1.96, 
  #                    conf.low1 = estimate - se*1.645, 
  #                    conf.high1 = estimate + se*1.645)
  ...
  dffinal <- rbind(dftwfe, dfcs, dfcsc, borus, sunab_tibble)
  return(dffinal)
}
...
writexl::write_xlsx(coefs_sent, '../../../results/estimates/sent_coefs_robustness.xlsx')

coefs_process <- c('total_processed', 'formal_prision', 'free') |> 
  map_dfr(~funct_atts(.x))

writexl::write_xlsx(coefs_process, '../../../results/estimates/processed_coefs_robustness.xlsx')
...
funct_atts_rate <- function(variable, control){
  ...
  borus <- did_imputation(data = p, yname = variable, 
                          gname = "event_time",
                          first_stage =as.formula(paste0('~ ', control, '| code_inegi + m_time')),
                          tname = "m_time", idname = "code_inegi") |> 
    mutate(type = "didimputation") |> 
    select(type, estimate, se = std.error) |>
    mutate( conf.low = estimate - se*1.96, 
            conf.high = estimate + se*1.96, 
            conf.low1 = estimate - se*1.645, 
            conf.high1 = estimate + se*1.645, variable = variable)
  
  print('borus done')
  
  
  dffinal <- rbind(dftwfe, dfcs, dfcsc, sunab_tibble, borus)
  return(dffinal)
}
...
results_plot <- ggplot(data = coefs_process, aes(x = estimate, y = factor(Variable))) + 
  ...
  geom_errorbar(aes(xmin = conf.low, xmax = conf.high, 
                ...
                linetype = 'solid') +
  scale_shape_manual(values = c(4, 15, 16, 7, 9), name = 'Estimator') +
  scale_color_manual(values = rep('black', 5), name = 'Estimator') +  # Set custom fill colors for points # Set custom line colors for error bars
  theme_bw() +  
  ylab("Variable") + 
...
ggsave(results_plot, 
       filename = paste0('../../../results/att/first_instance_robustness.pdf'), 
...
results_plot <- ggplot(data = coefs_sent, aes(x = estimate, y = factor(Variable))) + 
  ...
  geom_errorbar(aes(xmin = conf.low1, xmax = conf.high1, 
                    color = factor(type)), position = position_dodge(width = 0.7), 
                width = 0.8, linetype = "solid") +
  geom_errorbar(aes(xmin = conf.low, xmax = conf.high, 
                    color = factor(type)),
...
ggsave(results_plot, 
       filename = paste0('../../../results/att/second_instance_robustness.pdf'), 
