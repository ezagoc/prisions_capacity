...
event_general <- function(df, treat_post_p, time_period ='bimonthly', 
  ...
  if(time_period == 'monthly'){
    ...
    df <- df |> group_by(code_inegi, year, bimonthly) |> 
      summarise(across(c(crime_5, sent_prison), ~sum(.x))) |> 
    ...
    df <- df |> group_by(code_inegi, year) |> 
      summarise(across(c(crime_5, sent_intensive, sent_prison), ~sum(.x))) |> 
  ...
  df <- df |> mutate(sent_prison = log(sent_prison + 1))
...
do_plot(var = 'sent_prison', size_point = 1, size_titles = 13, time_b = -50, 
        time_a = 60, ylimb = .6, ylima = 0.3)
