...
event_general <- function(df, treat_post_p, time_period ='bimonthly', 
  ...
  df <- df |> select(code_inegi, CVE_ENT, year, month, semester, bimonthly, quarterly,
                     crime_5, sent_intensive, sent_intensive_incl, n_sentenced)
  if(time_period == 'monthly'){
    ...
    df <- df |> group_by(code_inegi, year, bimonthly) |> 
      summarise(across(c(crime_5, sent_intensive, sent_intensive_incl, n_sentenced), ~sum(.x))) |> 
  ...
  df <- df |> mutate(sent_intensive = log(ifelse(n_sentenced != 0, 
  ...
  fmla <- as.formula(paste0(variable, 
                            ' ~ i(time_to_event, treat, ref = -1) | code_inegi + m_time'))
  ...
  es <- did2s(df,
              yname = variable, first_stage = ~ 0 | code_inegi + m_time,
...
do_plot(var = 'sent_intensive', size_point = 1, size_titles = 13, time_b = -50, 
        time_a = 60, ylimb = .40, ylima = 0.40)
