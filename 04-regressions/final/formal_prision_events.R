...
event_general <- function(df, treat_post_p, time_period ='bimonthly', 
  ...
  if(time_period == 'monthly'){
    ...
    df <- df |> group_by(code_inegi, year, bimonthly) |> 
      summarise(across(c(all_of(variable)), ~sum(.x))) |> 
  ...
  df <- df |> mutate(across(c(all_of(all_of(variable))), ~log(.x + 1)))
...
do_plot(var = 'formal_prision', size_point = 1, size_titles = 13, time_b = -50, 
        time_a = 60, ylimb = .7, ylima = 0.4)
