...
pacman::p_load(tidyverse, arrow, purrr, sf, fixest, didimputation, did, DescTools)
...
event_general <- function(df, treat_post_p, time_period ='bimonthly', 
  ...
  ########### TWFE
  fmla <- as.formula(paste0(variable, ' ~ i(time_to_event, treat, ref = -1, keep = ', 
                            keept, ') ', 
                            addonvar, 
                            ' | code_inegi + m_time'))
  ...
  ########## Sun AB
  fmla <- as.formula(paste0(variable, ' ~ sunab(event_time, time_to_event, 
                            ref.p=c(-1, 49:72)) ',
                            addonvar,
                            ' | code_inegi + m_time'))
  reg200k <- feols(fmla, cluster = "code_inegi", data = df)

  pdf(paste0("../../results/events/test_judicial/sunab_",type_variable,
             '_', variable,'_', treat_post_p, '_',
             time_period, '_',winsor,  ".pdf"))
  iplot(reg200k)
  dev.off()
  ...
  ggsave(borus_plot, 
         filename = paste0("../../results/events/test_judicial/borus_",type_variable, 
                           '_', variable,'_', treat_post_p, '_', 
                           time_period,'_',winsor,  ".pdf"), 
  ...
  ggsave(sant, 
         filename = paste0("../../results/events/test_judicial/callsant_",type_variable, 
                           '_', variable,'_', treat_post_p, '_', 
                           time_period, '_',winsor, ".pdf"), 
         device = cairo_pdf, width = 9.22, height = 7, units = 'in')
...
# Now for robustness , check with 200 and 400

event_general(p, treat_post_p = 'btreat_200KM2',
              time_period ='bimonthly',
              variable = 'rate_sent_cond', 
              type_variable = 'levels')

event_general(p, treat_post_p = 'btreat_400KM2',
              time_period ='bimonthly',
              variable = 'rate_sent_cond', 
              type_variable = 'levels')

## Clear increase in the number of people condemned that are being sent to prison, instead of asked for money
...
# Finally lets go to the other code to check if this holds with controls

# Holds with controls

# Now checkk for quarterly or semester: 
...
# This is nice:
d <- event_general(p, treat_post_p = 'btreat_300KM2',
              time_period ='bimonthly',
              variable = 'rate_sent_cond', 
              type_variable = 'levels', winsor = 0.0)

event_general(p, treat_post_p = 'btreat_300KM2',
                   time_period ='bimonthly',
                   variable = 'rate_marg_formal_prision', 
                   type_variable = 'levels', winsor = 0.0)

event_general(p, treat_post_p = 'btreat_300KM2',
              time_period ='bimonthly',
              variable = 'rate_theft_formal_prision', 
              type_variable = 'levels', winsor = 0.0)

## Now lets try with normal variables

event_general(p, treat_post_p = 'btreat_300KM2',
              time_period ='bimonthly',
              variable = 'condenado', 
              type_variable = 'log', winsor = 0.05)

## Lets try with the dummies

event_general(p, treat_post_p = 'btreat_250KM2',
              time_period ='bimonthly',
